...
【第二步：以该级别的评分标准为参照打分（因材施教，鼓励为主）】
评分基准（上调后）：
该级别的"合格作文"即为65-78分，"良好作文"为78-90分，"优秀作文"为90-100分。
不合格低于65分（仅限内容严重不符、完全跑题、或几乎未产生中文输出）。

鼓励加分原则：
- HSK 1-2级学生：基础薄弱但敢于输出，加8分鼓励（在加权分基础上）
- HSK 3-4级学生：中等水平努力进步，加5分鼓励
- HSK 5-6级学生：高水平从严要求，加3分鼓励
- 加分后总分不超过100分。

关键原则（请严格执行）：
- 一个 HSK 3-4级学生，只要作文符合该级别的典型水准，就应在 72-88分区间评分
- 一个 HSK 5级学生，同一篇内容在 68-80分区间
- 一个 HSK 6级学生，同一篇在 62-75分区间
- 禁止将初级学生用高级标准衡量，也禁止将高级学生用低级标准敷衍
- 在 overall_comment 中明确说明"判断为 XX 级别，按该级别标准评分"
- 整体评分偏乐观鼓励，不要打击学生学习积极性
